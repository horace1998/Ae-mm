...
export default function RitualDashboard() {
  ...
  return (
    <div className="w-full h-full flex flex-col p-6 md:p-12 overflow-y-auto gap-8 md:gap-24 custom-scrollbar relative pb-32">
      
      {/* Editorial Header Section */}
      <section className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-synk-border pb-8 md:pb-12">
        <div className="flex flex-col gap-1">
          <h1 className="high-fashion-header text-4xl md:text-7xl leading-none">PORTAL_SYNC</h1>
          <p className="text-[8px] md:text-[10px] tracking-[0.4em] md:tracking-[0.5em] text-synk-lavender font-light uppercase ml-0.5 md:ml-1">
            虛擬同步中心 <span className="opacity-20 ml-2">ACCESS_LEVEL_O1</span>
          </p>
        </div>
        ...
        <div className="flex flex-col items-start md:items-end gap-3 w-full md:w-auto">
          <div className="flex items-center justify-between md:justify-end gap-6 w-full md:w-auto">
            <div className="flex flex-col items-start md:items-end">
               <span className="text-[8px] md:text-[9px] uppercase tracking-[0.3em] text-white/30">RESONANCE</span>
            ...
            <div className="flex flex-col items-start md:items-end">
               <span className="text-[8px] md:text-[9px] uppercase tracking-[0.3em] text-white/30">AURA_RATING</span>
          ...
          <motion.div 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="text-[8px] md:text-[10px] uppercase tracking-[0.3em] text-synk-pink bg-synk-pink/10 px-3 py-1 rounded w-fit md:ml-auto"
          >
      ...
      <section className="bg-synk-glass border border-synk-border rounded-[24px] md:rounded-[40px] p-6 md:p-12 relative flex flex-col items-center justify-center min-h-[400px] md:min-h-[480px] overflow-hidden group shadow-[0_40px_100px_rgba(0,0,0,0.3)]">
        ...
        <div className="absolute top-6 left-6 md:top-12 md:left-12 z-20 flex flex-col pointer-events-none">
          <span className="hidden md:block text-[9px] uppercase tracking-[0.4em] text-white/30 mb-4 vertical-text h-32 origin-top-left">UNIVERSE_WHISPER</span>
         ...
         <div className="mt-auto mb-6 md:mb-12 text-center flex flex-col items-center gap-2 z-10 w-full px-6 md:px-12">
          <div className="flex flex-col md:flex-row justify-between w-full items-center md:items-end gap-6 md:gap-0">
            ...
            >
              ACCESS_DESIGNER <span className="hidden xs:inline">/ 空間配置</span>
            </motion.button>

            <div className="flex flex-col items-center md:items-end order-3">
               <span className="text-[8px] md:text-[10px] uppercase tracking-[0.4em] text-white/40">MEMBER_CODE</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12 pb-12">
         ...
         <div className="flex flex-col gap-6 p-6 rounded-2xl bg-white/2 border border-white/5">
            ...
            <div className="flex items-center gap-6">
               ...
               <div className="flex flex-col gap-1 min-w-0">
                  <span className="text-lg font-serif uppercase tracking-widest italic text-white leading-none truncate">Agent_MY</span>
         ...
         <div className="flex flex-col gap-6 p-6 rounded-2xl bg-white/2 border border-white/5">
            <div className="flex items-center justify-between border-b border-white/10 pb-4">
               ...
               <span className="text-[10px] font-mono text-white/20 text-synk-blue">MAX_SYNC</span>
            </div>
            ...
            >
               <div className="flex flex-col gap-12 h-full">
                  <header className="flex flex-col gap-2">
                     ...
                     <p className="text-[10px] text-synk-lavender uppercase tracking-[0.4em] font-light">宇宙設計中心 / <span className="text-white/30">UNIVERSE_EDITOR</span></p>
                  </header>
                  ...
                  <div className="flex flex-col gap-6 flex-1">
                     <div className="flex items-center justify-between border-b border-white/5 pb-4">
                        <span className="text-[10px] uppercase tracking-[0.4em] text-white/20">正在具現化 / ACTIVE_SLOTS</span>
                        ...
                        {decorations.map((dec) => (
                           <div key={dec.id} className="flex items-center gap-6 border-b border-white/5 pb-4 group">
                              ...
                              <div className="flex flex-col flex-1">
                                 <span className="text-[12px] font-serif italic text-white/80">{dec.type === 'image' ? 'Visual_Shard' : 'Energy_Crystal'}</span>
                                 <span className="text-[8px] uppercase tracking-widest text-white/20">ID_{dec.id?.slice(0, 6)}</span>
                              </div>
      ...
      <div className="bg-synk-glass border border-synk-border rounded-sm p-12 flex flex-col gap-14 group">
         <div className="flex flex-col gap-2 items-start border-b border-white/10 pb-12 w-full">
            <h4 className="high-fashion-header text-4xl">USER_SYNC</h4>
            <p className="text-[10px] text-synk-lavender uppercase tracking-[0.5em] font-light">成員同步模式 / <span className="text-white/30">SELECT_RESONANCE_TARGET</span></p>
         </div>
            ...
            {(['None', 'Karina', 'Winter', 'Giselle', 'Ningning'] as const).map(m => (
               ...
               >
                  ...
                  <div className="flex flex-col items-center gap-1">
                    ...
                    {bias === m && (
                      <span className="text-[8px] uppercase tracking-widest text-synk-lavender">Active_Link</span>
                    )}
                  </div>
               </button>
            ))}
