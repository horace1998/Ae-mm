...
export default function GoalVault() {
  ...
  return (
    <div className="w-full h-full p-8 lg:p-14 pb-48 overflow-y-auto custom-scrollbar">
      <div className="flex flex-col gap-16 md:gap-24">
        ...
        <header className="flex flex-col gap-12 border-b border-white/10 pb-8 md:pb-12">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-8">
            <div className="flex flex-col">
              <span className="text-[9px] md:text-[10px] tracking-[0.4em] md:tracking-[0.5em] text-white/20 mb-3 md:mb-4 block uppercase font-light">DATABASE_VAULT_V4</span>
              <h1 className="high-fashion-header text-5xl md:text-8xl leading-none">VAULT</h1>
              <div className="flex items-center gap-3 md:gap-4 mt-2">
                <span className="text-[9px] md:text-[10px] tracking-[0.4em] text-synk-lavender uppercase font-light leading-none">應援數據庫</span>
                <span className="w-8 md:w-12 h-[1px] bg-synk-lavender/30" />
                <span className="text-[9px] md:text-[10px] tracking-[0.4em] text-white/30 uppercase leading-none">{getLoreTitle()}</span>
              </div>
            </div>
            
            <div className="flex flex-col items-start md:items-end gap-4 w-full md:w-auto">
              <span className="text-[8px] md:text-[9px] uppercase tracking-[0.4em] text-white/30">VIEW_MODE / 檢視模式</span>
              <div className="flex bg-white/2 p-1.5 md:p-2 rounded-sm border border-white/5 backdrop-blur-md w-full md:w-auto overflow-x-auto no-scrollbar">
        ...
        {/* Input Block: EXECUTION_PROTOCOL */}
        <section className="flex flex-col gap-8 max-w-4xl">
          <div className="flex items-start md:items-center gap-4">
            <span className="hidden md:block text-[10px] uppercase tracking-[0.4em] text-synk-pink vertical-text h-32 origin-top-left pt-2">PROTOCOL_01</span>
            <div className="flex-1">
               <h3 className="text-xl md:text-2xl font-serif italic text-white/90 mb-4 md:mb-6 tracking-tight">「在此輸入您的靈魂指令」</h3>
               <form onSubmit={handleAdd} className="flex flex-col gap-6 md:gap-8 bg-white/2 p-6 md:p-10 border border-white/5 hover:border-white/20 transition-all rounded-xl md:rounded-3xl">
                <div className="flex items-center gap-4 md:gap-8 w-full border-b border-white/10 pb-4 group">
                  <span className="hidden xs:block text-[9px] md:text-[10px] tracking-[0.3em] text-white/20 group-focus-within:text-white transition-colors">ENTRY_ID:</span>
                  <input
                    type="text"
                    value={newTitle}
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-8">
                  <div className="flex flex-col gap-4">
                    <span className="text-[8px] md:text-[9px] uppercase tracking-[0.3em] text-white/30">SELECT_PHASE / 階段</span>
        ...
        <section className="flex flex-col gap-12">
          <div className="flex items-center gap-6">
             <h2 className="high-fashion-header text-3xl">DIRECTIVES</h2>
             <span className="w-px h-6 bg-white/10" />
             <span className="text-[10px] tracking-[0.4em] text-white/30 uppercase">當前指令集 / {goals.length} SLOTS_ACTIVE</span>
          </div>
                    ...
                    >
                      <div className="flex items-center gap-6 md:gap-12 w-full min-w-0">
                        ...
                        <div className="flex flex-col gap-1 min-w-0 flex-1">
                          ...
                          <span className="text-[8px] md:text-[9px] uppercase tracking-[0.4em] text-white/20">AGENT_PROTOCOL_ID: {goal.id.slice(0, 8)}</span>
                        </div>
                      </div>
                      
                      <div className="flex flex-row md:flex-col items-center md:items-end gap-3 md:gap-1 w-full md:w-auto mt-2 md:mt-0">
                         <span className={cn("text-[9px] md:text-[10px] uppercase tracking-[0.3em] font-medium px-2 py-0.5 rounded md:p-0 md:bg-transparent", getGoalColor(goal.type))}>PHASE_{goal.type}</span>
                         <span className="text-[8px] uppercase tracking-widest text-white/10 ml-auto md:ml-0">STABILITY_CHECK: OK</span>
                      </div>
