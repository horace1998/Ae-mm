...
export default function ThreeBackground({ completionRate }: { completionRate: number }) {
  ...
  useEffect(() => {
    ...
    const orbMat = new THREE.MeshStandardMaterial({ 
      color: 0x222222,
      emissive: new THREE.Color().lerpColors(new THREE.Color("#60a5fa"), new THREE.Color("#f472b6"), completionRate),
      emissiveIntensity: 0.5 + completionRate, // More complete = brighter
      roughness: 0.2,
      metalness: 0.8
    });
    ...
    const ring1Mat = new THREE.MeshBasicMaterial({ color: 0xa78bfa, transparent: true, opacity: 0.6 });
    const ring1 = new THREE.Mesh(ring1Geo, ring1Mat);
    ring1.rotation.x = Math.PI / 2;
    group.add(ring1);

    const ring2Geo = new THREE.TorusGeometry(4, 0.02, 16, 100);
    const ring2Mat = new THREE.MeshBasicMaterial({ color: 0x60a5fa, transparent: true, opacity: 0.3 });
    ...
    const partsMat = new THREE.PointsMaterial({ color: 0xffffff, size: 0.05, transparent: true, opacity: 0.5 });
