...
import { Sparkles, Vault, Zap, Archive as ArchiveIcon, Fingerprint } from "lucide-react";
import { cn } from "./utils";
import RitualDashboard from "./tabs/RitualDashboard";
import GoalVault from "./tabs/GoalVault";
import PurgeMode from "./tabs/PurgeMode";
...
const TABS = [
  ...
  { id: "vault", label: "VAULT", subLabel: "核心指令", icon: Vault },
  { id: "purge", label: "WHIPLASH", subLabel: "重構協議", icon: Zap },
  { id: "archive", label: "JOURNAL", subLabel: "數據檔案", icon: ArchiveIcon },
...
export default function AppLayout() {
  ...
  return (
    <div className={cn(
      ...
      <div className={cn(
        "absolute inset-0 pointer-events-none z-[-5] overflow-hidden transition-opacity duration-1000",
        roomAtmosphere === 'Void' ? 'opacity-20' : 'opacity-100'
      )}>
        <motion.div 
          animate={{ 
            y: [0, -40, 0],
            ...
            >
              <div 
                className={cn(
                  ...
                  tab.id === "purge" ? "bg-synk-pink text-white border-transparent" : "border-current",
                  isActive && tab.id !== "purge" ? "bg-white/10 border-white" : "group-hover:border-white/40"
                )}
              >
